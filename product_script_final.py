...
# -----------------------------------------------------------------------------------------
# --- 2. Write Data to CSV and Convert to TypeScript (Integrated Workflow) ---
# -----------------------------------------------------------------------------------------
...
if __name__ == "__main__":
    try:
        # Step 1: Write initial data to CSV
        with open(csv_file_path, 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(products)
        print(f"CSV template created/overwritten with {len(products)} products: {os.path.abspath(csv_file_path)}")

        # Step 2: Read from CSV and Write to TypeScript
        ...
        print("\n✅ Data is ready for display. Run 'pnpm run dev'.")
